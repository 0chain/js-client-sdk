...
module.exports = {
  ...
  createWalletAndDesiredAllocation: async function () {
    ...
    return await this.registerClient()
      ...
      .then((response) => {
        ...
        return this.allocateStorage(
          responseObj.activeWallet,
          dataShards,
          parityShards,
          allocationSize,
          tokenLock
      ...
      .then((response) => {
        ...
        return this.lockTokensInReadPool(
          responseObj.activeWallet,
          response.hash,
          2592000000000000,
          5000000000
  ...
  getChainStats: () => {
    return getInformationFromRandomSharder(
  ...
  getBlockInfoByHash: function getBlockInfoByHash(
    hash,
    options = this.BlockInfoOptions.HEADER
  ) {
    const blockInfoOptions = this.BlockInfoOptions;
    return getInformationFromRandomSharder(
      Endpoints.GET_BLOCK_INFO,
      { block: hash, content: options },
      (rawData) => {
  ...
  getBlockInfoByRound: function getBlockInfoByRound(
    round,
    options = this.BlockInfoOptions.HEADER
  ) {
    const blockInfoOptions = this.BlockInfoOptions;
  ...
  getBalance: (client_id) => {
    return new Promise(async function (resolve, reject) {
      utils
        .getConsensusedInformationFromSharders(
          sharders,
  ...
  allocationMinLock: function allocationMinLock(
    ae,
    data,
    parity,
    size,
    ...
    expiration_date = Math.floor(
      expiration_date.addDays(expiration_days).getTime() / 1000
    );

    return utils.getConsensusedInformationFromSharders(
      sharders,
      Endpoints.SC_REST_ALLOCATION_MIN_LOCK,
      {
        allocation_data: {
          data_shards: data,
          ...
          .then((res) => {
            ...
            if (url != 'blobbersList') {
              ...
              activeUrls = res.data.Nodes.filter(
                (value) =>
                  new Date().getTime() -
                    new Date(value.last_health_check * 1000).getTime() <
                  3600000
              );
            }
  ...
  getStakeLockedToken: (client_id) => {
    return new Promise(async function (resolve, reject) {
      let minerSharderStake = await utils
        .getConsensusedInformationFromSharders(
          sharders,
          Endpoints.GET_MINERSC_POOL_STATS,
          { client_id }
        )
        .then((res) => {
          return res;
      ...
      let blobberStake = await utils
        .getConsensusedInformationFromSharders(
          sharders,
          Endpoints.GET_STORAGESC_POOL_STATS,
          { client_id }
        )
        .then((res) => {
          return res;
      ...
      resolve({
        pools: {
          ...minerSharderStake.pools,
          blobber: { ...blobberStake.pools },
        },
      });
    });
  },
  ...
  getAllBlobbers: function getAllBlobbers() {
    return utils
      .getConsensusedInformationFromSharders(
        sharders,
        Endpoints.SC_BLOBBER_STATS,
        {}
      )
      .then((res) => {
        const response = res.Nodes.filter(
          (value) =>
            new Date().getTime() -
              new Date(value.last_health_check * 1000).getTime() <
            3600000
  ...
  getAllocationSharedFilesFromPath: async function (
    ...
    const list = await utils.getReqBlobbers(
      blobber_url,
      { path_hash: lookup_hash, auth_token: auth_token },
      client_id
    );
  ...
  getAllocationFilesFromHash: async function (
    allocation_id,
    lookup_hash,
    client_id
  ) {
  ...
  getFileMetaDataFromPath: async function (allocation_id, path, client_id) {
    ...
    return new Promise(async function (resolve, reject) {
      const blobber_url =
        blobber + Endpoints.FILE_META_ENDPOINT + allocation_id;
      const response = await utils.postReqToBlobber(
        blobber_url,
        {},
        { path: path },
        client_id
  ...
  getFileStatsFromPath: async function (allocation_id, path, client_id) {
    ...
    return new Promise(async function (resolve, reject) {
      ...
      for (let blobber of completeAllocationInfo.blobbers) {
        const blobber_url =
          blobber.url + Endpoints.FILE_STATS_ENDPOINT + allocation_id;
        await utils
          .postReqToBlobber(blobber_url, {}, { path: path }, client_id)
          .then((response) => {
  ...
  getFileMetaDataFromPathHash: async function (
    allocation_id,
    path_hash,
    auth_ticket,
    client_id
    ...
    return new Promise(async function (resolve, reject) {
      ...
      const response = await utils.postReqToBlobber(
        blobber_url,
        {},
        { path_hash: path_hash, auth_token: atob(auth_ticket) },
        client_id
      );
      if (response.status === 200) {
        const res = {
  ...
  commitMetaTransaction: async function (
    ae,
    crudType,
    allocation_id,
    path = '',
    ...
    if (metadata.length === 0) {
      if (path.length > 0) {
        metadata = await this.getFileMetaDataFromPath(
          allocation_id,
          path,
          ae.id
        );
        ...
        metadata = await this.getFileMetaDataFromPathHash(
          at.allocation_id,
          lookuphash,
          auth_ticket,
          ae.id
    ...
    await this.updateMetaCommitToBlobbers(
      submitResponse.hash,
      allocation_id,
      transactionData.MetaData.LookupHash,
      ae.id,
      auth_ticket
    );
  ...
  updateMetaCommitToBlobbers: async function (
    transaction_hash,
    allocation,
    lookup_hash,
    client_id,
  ...
  downloadSharedObject: async function (
    allocation_id,
    auth_ticket,
    lookup_hash,
    file_name,
  ...
  encryptPublicKey: async function (client_json) {
    const url =
      proxyServerUrl + Endpoints.PROXY_SERVER_ENCRYPT_PUBLIC_KEY_ENDPOINT;
    const params = {
      client_json: client_json,
    };
    const response = await utils.getReq(url, params);
  ...
  recoverWalletFromCloud: async function (AppIDToken, AppPhoneNumber) {
    ...
    const response = await utils.recoverWalletFromCloud(
      url,
      AppIDToken,
      AppPhoneNumber
    );
  ...
  saveWalletToCloud: async function (
    activeWallet,
    encryptMnemonicUsingPasscode,
    tokenId,
    phone
  ) {
  ...
  createFree2GbAllocation: async function (
    id_token,
    phone_num,
    encryption_key,
    username,
  ...
  deleteExistAllocation: async function (
    id_token,
    phone_num,
    client_id,
    client_key
  ...
  getShareInfo: async function (activeWallet) {
    const url = zeroBoxUrl + Endpoints.ZEROBOX_SERVER_SHARE_INFO_ENDPOINT;
    const clientSignature = this.getSign(
      activeWallet.id,
      activeWallet.secretKey
    );
    const response = await utils.getShareInfo(
      url,
      clientSignature,
      activeWallet.id,
      activeWallet.client_key
  ...
  executeFaucetSmartContract: function (
    ae,
    methodName,
    input,
    transactionValue
...
function createWallet(mnemonic) {
  ...
  return new Promise(function (resolve, reject) {
    utils
      .doParallelPostReqToAllMiners(miners, Endpoints.REGISTER_CLIENT, data)
      .then((response) => {
